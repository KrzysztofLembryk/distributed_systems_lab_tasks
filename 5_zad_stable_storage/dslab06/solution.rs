...
impl Storage
{
    ...
    ) -> std::io::Result<()>
    {
        ...
        // Since we know that data is at most BUF_SIZE + HASH_SIZE 
        // we can safely read whole file to memory
        let data = t_fs::read(tmp_file_path).await.expect("check_tmp_file - tokio::fs::read failed");

        if data.len() > BUF_SIZE + HASH_SIZE
        {
            panic!("File: '{tmp_file_name}' is too large, this shouldn't have happened");
        }

        // Hash plus at least one byte of data
        if data.len() < HASH_SIZE + 1
        {
            panic!("File: '{tmp_file_name}' is too small, this shouldn't have happened");
        }
    ...
    {
        // We assume that we get path from self.file_keys
