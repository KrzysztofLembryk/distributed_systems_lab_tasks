...
pub(crate) mod tests {
    ...
    use crate::domain::{
        CHORD_FINGER_TABLE_MAX_ENTRIES, CHORD_RING_TABLE_MAX_ENTRIES, ChordId, ChordLinkId,
        ChordRoutingState, Internet, chord_id_advance_by, chord_id_distance, chord_id_max,
    };
    use crate::solution::ChordNode;
    use crate::{
        ChordSystemConfig, chord_route_from_node_to_node,
        wire_all_chord_nodes_given_global_info_from_oracle,
    };
    ...
    async fn routing_in_full_four_bit_ring_with_no_redundancy_should_use_all_links() {
        ...
        for &src_id in cfg.all_nodes.keys() {
            for &dst_id in cfg.all_nodes.keys() {
                let num_expected_hops = if chord_id_advance_by(cfg.ring_bits, &dst_id, &1) == src_id
                {
    ...
    pub(crate) fn build_chord_node_routing_state<const REDUNDANCY: usize, const FINGERS: usize>(
        ...
        for i in 0..succs.len() {
            ...
            succ_table.push(Some(ChordLinkId {
                id: succs[i],
                addr: *cfg.all_nodes.get(&succs[i]).unwrap(),
            }));
            pred_table.push(Some(ChordLinkId {
                id: preds[i],
                addr: *cfg.all_nodes.get(&preds[i]).unwrap(),
            }));
        }
        ...
        for (idx, id) in fingers {
            ...
            assert!(finger_table[idx].is_none());
            finger_table[idx] = Some(ChordLinkId {
                id,
                addr: *cfg.all_nodes.get(&id).unwrap(),
            });
        }
        ChordRoutingState {
            finger_table,
    ...
    fn check_chord_node_link_vector_match(
        id: &ChordId,
        vec_name: &str,
        actual_vec: &[Option<ChordLinkId>],
        expected_vec: &[Option<ChordLinkId>],
    ) {
        assert!(
